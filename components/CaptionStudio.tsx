...
export default function CaptionStudio() {
  ...
  async function transcribe() {
    ...
    setBusy(true);
    setStatus("Uploading your video securely to CaptionAI…");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(detail || `Caption service returned HTTP ${response.status}.`);
      }

      const data = await response.json().catch(() => null);
      if (!data || !Array.isArray(data.segments)) {
        throw new Error("Caption service returned an unexpected response. Please try again.");
      }

      setSegments(data.segments);
      setStatus(
        data.segments.length
          ? `Done — ${data.segments.length} ${selectedLanguage} caption segments generated by Gemini.`
          : "No understandable speech was found in this video. Try a video with clearer audio."
      );
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : "Unknown error";
      if (error instanceof TypeError && /fetch/i.test(message)) {
        setStatus("We couldn't connect to CaptionAI. Please check your internet connection and try again.");
      } else if (/Failed to fetch|NetworkError|Load failed/i.test(message)) {
        setStatus("We couldn't reach the caption service. Please try again in a moment.");
      } else {
        setStatus(message || "Caption generation failed. Please try again.");
      }
    } finally {
      setBusy(false);
    }
