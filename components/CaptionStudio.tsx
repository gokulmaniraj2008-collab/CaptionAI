...
export default function CaptionStudio() {
  ...
  async function transcribe() {
    ...
    try {
      ...
      if (!res.ok) {
        const serverMessage = data.error || `Request failed with HTTP ${res.status}`;
        if (/request entity too large|payload too large|body size|413/i.test(serverMessage) || res.status === 413) {
          throw new Error("This video is too large for the current upload route. Please try a smaller video (for example, under 20 MB). The server rejected the upload before Gemini could process it.");
        }
        throw new Error(serverMessage);
      }

      if (!data.segments) throw new Error(data.error || "Gemini returned an unexpected response.");
